// CORREÇÃO: Centralizar todas as funções utilitárias
...
// Formatação de data - CORRIGIDA COMPLETAMENTE PARA EVITAR PROBLEMAS DE TIMEZONE
export function formatDate(date: string | Date | null | undefined): string {
  if (!date) return "-"

  try {
    // Se é uma string, fazer parse manual SEMPRE
    if (typeof date === "string") {
      // Extrair apenas YYYY-MM-DD (ignora tudo depois de T)
      ...
      if (match) {
        ...
        // Retornar diretamente no formato DD/MM/YYYY sem conversão de timezone
        return `${day}/${month}/${year}`
      }
    }

    // Se é Date object, extrair componentes diretamente
